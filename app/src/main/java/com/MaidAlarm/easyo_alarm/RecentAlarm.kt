...
class RecentAlarm {
    fun checkSQL(SQLHelper : SQLHelper): MutableList<Int> {
        ...
        // 두 번째로 작은 숫자를 골라낸다
        val secondSmall = Function().secondSmall(timeList, recentTime)

        val resultIdxInSwitch = timeList.indexOf(recentTime)    // 여기에 switch on 리스트 안에서 제일 가까운 알람의 인덱스가 들어간다
        val secondSmallIndex = timeList.indexOf(secondSmall)    // 두번째로 가까운 알람의 인덱스가 들어간다
        ...
        // requestCode_switch[resultIdxInSwitch]: 가장 가까운 알람의 requestCode 값
    ...
    // 첫 번째 또는 두 번째로 가까운 알람에 대한 정보를 SQL에서 가져온다
    // FrontActivity 에서 완전히 ok버튼을 눌었을 때 시간 = 제일 가까운 알람의 시간(분) -> 두 번째 가까운 알람 반환
    // FrontActivity 에서 완전히 ok버튼을 눌었을 때 시간 != 제일 가운 알람의 시간(분) -> 제일 가까운 알람 반환
    // 아래 함수는 FrontActivity에서 완전히 ok 버튼을 누른 후 호출되는 함수이다
    fun checkSQLGetSecond(SQLHelper : SQLHelper): MutableList<Int> {
        val timeList = mutableListOf<Int>() // 여기에 SQL 데이터에서 뽑아낸 시간 차이 정보가 들어간다
        ...
        // 전체 알람 SQL 데이터에서 두번째로 가까운 알람에 대한 인덱스가 들어간다
        // requestCode_switch[resultIdxInSwitch]: 가장 가까운 알람의 requestCode 값
        var resultIdx = requestCode.indexOf(requestCode_switch[resultIdxInSwitch])
